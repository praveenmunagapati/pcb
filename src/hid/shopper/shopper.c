 ...
 *
 * \author Copyright (C) 2017 for adaptation by Bert Timmerman
 ...
 *
 * \warning Please use the exact order as the countr_names array from
...
/* %start-doc options "95 pcbshopper.com RFQ Options"
@ftable @code
@item --unit <unit>
...
Parameter @code{<string>} can be @samp{"1 oz"}, @samp{"2 oz"},
@samp{"3 oz"}, @samp{"4 oz"}, @samp{"5 oz"}, or @samp{"6 oz"}.
@end ftable
%end-doc
*/
...
Minimum trace width in units as defined with '--unit'.
@end ftable
%end-doc
*/
...
Minimum space between traces in units as defined with '--unit'.
@end ftable
%end-doc
*/
...
Minimum drill width in units as defined with '--unit'.
@end ftable
%end-doc
*/
...
@ftable @code
@item --quantity <int>
Where @code{<int>} is the quantity of boards. Up to 1,000.
@end ftable
%end-doc
*/
...
/* shopper variables. */
char *shopper_version = "1.0";
char *shopper_warnings;
...
char *shopper_quote_URL;
char *shopper_param_warnings;

/* Variables for the pcbShopper API. */
char *shopper_units; /*!< Dimension measurement units. */
...
char *shopper_tracewidth_units; /*!< Trace width dimension measurement units. */
double shopper_min_drill_size; /*!< Minimum drill size. */
char *shopper_drillwidth_units; /*!< Drill width dimension measurement units. */
...
char *shopper_country; /*!< Country to ship to. */
...
shopper_assemble_quote_URL (void)
{
  ...
  /* Create string for copper surface finish. */
  ...
  /* Create string for quality certificates. */
...
{
  char tSilk = false; /* Set Top silk flag to false. */
  char bSilk = false; /* Set Bottom silk flag to false. */
  char tFinish = false; /* Set Top finish flag to false. */
  char bFinish = false; /* Set Bottom finish flag to false. */
  ...
  if (PCB->Data->RatN != 0)
    {
      Message (_("shopper: unrouted nets found, you haven't finished yet !\n"));
    }

  /* Copper layer count.
   * Loop over each layer(group) and count each copper layer used. */
  ...
  /* Get the board dimensions from the outline layer. */
  ...
  /* Loop over copper arcs and find minimum width. */
  ...
  /* Hard gold finish objects.
   * Basically we are looking for traces near the board edge on the
   * outer layers which are, not covered with solder mask and have the
   * attribute "finish" set to "ENIG", or fiducials. */

  /*! \todo Add code for ENIG fingers and other plated objects. */

  /* Defauls for solder mask color, copper finish type,
   * copper weight, stencil, quality options, design number, country and
   * lead time for delivery in days. */
  shopper_soldermask_color_index = 0;
  shopper_copper_finish_type = 0;
  shopper_copper_weight_index = 0; /* 1oz. */
  shopper_stencil = 0;
  shopper_quality = 0; /* No quality check. */
  shopper_designs = 1; /* Always at least one design.*/
  shopper_quantity = 3; /* So OSHpark is always an option. */
  shopper_country = "US"; /* Assumed US. */
  shopper_days = 0; /* Default. */
}
